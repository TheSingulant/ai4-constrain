...
Open over **HTTPS** in Chrome with Phantom (jsDelivr GitHub CDN at the
commit SHA). Phantom injects `window.phantom.solana` on https. `file://`
does not work. `http://127.0.0.1` is an optional offline/dev fallback only.
